...
async def iva(interaction: discord.Interaction, prompt: str, file: discord.Attachment=None):
    ...
    try:
        ...
        - Always use LaTeX formatting for every mathematical expression, no matter how simple or complex. Wrap all LaTeX code in double dollar signs `$$` and place it on a new line, like this: `\\n$$[latex]$$`. This should be done even for expressions that do not strictly require LaTeX formatting. Please apply LaTeX formatting to tables and other complex information displays as well.
